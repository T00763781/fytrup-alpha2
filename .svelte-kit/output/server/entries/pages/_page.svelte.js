...
const DialoguePanel = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  portraitSrc = `${base}${portrait}`;
  return `<div class="${"panel " + escape(visible ? "visible" : "", true) + " svelte-ydgxoh"}"><div class="header svelte-ydgxoh"><img class="portrait svelte-ydgxoh"${add_attribute("src", portraitSrc, 0)}${add_attribute("alt", character, 0)}> <div class="character svelte-ydgxoh">${escape(character)}</div></div> <div class="title svelte-ydgxoh">${escape(title)}</div> <div class="body svelte-ydgxoh">${escape(text)}</div> <button class="btn svelte-ydgxoh" data-svelte-h="svelte-1tlj5zy">Continue</button></div>`;
});
